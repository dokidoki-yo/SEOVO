...
def cam2pixel2(cam_coords, proj_c2p_rot, proj_c2p_tr, padding_mode):
    ...
    pixel_coords = torch.stack([X_norm, Y_norm], dim=2)  # [B, H*W, 2]
    pixel_coords_origin = torch.stack([X_origin, Y_origin], dim=2)  # [B, H*W, 2]
    return pixel_coords.reshape(b, h, w, 2), Z.reshape(b, 1, h, w), pixel_coords_origin.reshape(b, h, w, 2)
...
def inverse_warp2(img, depth, ref_depth, pose, intrinsics, padding_mode='zeros'):
    ...
    rot, tr = proj_cam_to_src_pixel[:, :, :3], proj_cam_to_src_pixel[:, :, -1:]
    src_pixel_coords, computed_depth, src_pixel_coords_origin= cam2pixel2(cam_coords, rot, tr, padding_mode)  # [B,H,W,2]
    ...
    return projected_img, src_pixel_coords, src_pixel_coords_origin, valid_mask, projected_depth, computed_depth
